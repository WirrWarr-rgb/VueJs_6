  ...
  <div class="container mx-auto px-4 py-8">
    <div class="max-w-md mx-auto">
      ...
      <div v-if="isLoading" class="text-center py-8">
        ...
        <p class="mt-2 text-gray-600">Загрузка категории...</p>
      </div>
...
// Загрузка категории через useQuery (ТРЕБОВАНИЕ)
const { data: category, isLoading } = useQuery({
  queryKey: ['category', props.id],
  queryFn: () => categoriesApi.getCategory(props.id).then(res => res.data),
  enabled: !!props.id
})
...
// Обновление категории через useMutation (ТРЕБОВАНИЕ)
